...
class _ResolvedTourGroup {
  ...
  TourModel get representative {
    for (final tour in tours) {
      if (tour.id == representativeTourId) {
        return tour;
      }
    }
    return tours.first;
          ...
          children: [
            ...
            if (tour.departureDays.isNotEmpty) ...[
              ...
              _infoItem(
                'Haftalık Günler',
                tour.departureDays
                    .map((day) => _TourScheduleScreenState._dayLabels[day] ?? '')
                    .join(', '),
              ),
            ],
...
class _ScheduleTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dateText = _formatScheduleDate(date);
  ...
  String _formatScheduleDate(DateTime value) {
    final dayLabel = _TourSummaryCard._shortDayLabels[value.weekday] ?? '';
    return '${DateFormat('dd.MM.yyyy').format(value)}${dayLabel.isEmpty ? '' : ' • $dayLabel'}';
  }
}
